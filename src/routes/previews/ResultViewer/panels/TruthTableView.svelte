<script>
  ...
  import { padding } from "../../CircuitViewer/svg_utils/constants";
  ...
  {#if $drawPlan?.groups}
    ...
    >
      <div
        ...
        style={`width: ${$drawPlan.width + padding * 2}px; height:${$drawPlan.groups.header_group.height + padding}px;`}
      >
        <svg
          id={id + "-sticky"}
          width={$drawPlan.width + padding * 2}
          height={$drawPlan.groups.header_group.height + padding}
          viewBox={[
            0,
            0,
            $drawPlan.width + padding * 2,
            $drawPlan.groups.header_group.height + padding,
          ].join(" ")}
        >
          <style>
            g {
          ...
          {#if $drawPlan.groups.header_group}
            <SvgWrap data={$drawPlan.groups.header_group}></SvgWrap>
          {/if}
        </svg>
      </div>
      <div class="count-histogram-main">
        ...
        >
          ...
          {#if $drawPlan.groups.header_group}
            <SvgWrap data={$drawPlan.groups.header_group}></SvgWrap>
          {/if}
          {#if $drawPlan.groups.item_group}
            <SvgWrap
              data={$drawPlan.groups.item_group}
              {openTooltip}
              {closeTooltip}
              {moveTooltip}
            ></SvgWrap>
