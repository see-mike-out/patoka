            ...
            {#if (!filter.gates || filter.gates.length == 0 || (filter.gates && filter.gates.includes(gate.gate))) && (!filter.qubits || filter.qubits.length == 0 || (filter.qubits && gate.qubits.some( (q) => filter.qubits.includes(q), )))}
              <tr>
                ...
                <td>
                  ...
                  <span class="asof">{gate.parameters.gate_error?.asof}</span>
                </td>
                <td>
                  {gate.parameters.gate_length?.value} ({gate.parameters
                    .gate_length.unit})<br />
                  <span class="asof">{gate.parameters.gate_length?.asof}</span>
                </td>
                <td>
                  {#if addToBasket}
                    <button
                      ...
                      on:click={() => {
                        addToBasket(
                          `gate_${gate.gate}_${gate.qubits.join("_")} = ${code_header}gate="${gate.gate}", qubits=[${gate.qubits.join(", ")}]${code_footer}`,
                        );
                      }}
                    >
                      <Cart
                  ...
                  <td>
                    <button
                      ...
                      on:click={() => {
                        openTimeMachine({
                          datatype: "gates",
                          key: gate.gate + "_" + gate.qubits.join("_"),
                          title_key: `gate[${gate.qubits.join(",")}].${gate.gate}`,
                          index: gi,
                          parentKey,
                          topLevelKey,
                          data: gate,
